...
def campo_moneda(label, campo_key, ayuda="", valor_defecto=0):
    """Campo de dinero con formato de miles. Guarda el TEXTO formateado
    directamente bajo su propia llave de session_state -sin una llave
    numérica separada-, así Streamlit lo persiste solo, sin importar
    cuántas veces cambies de pestaña. El valor numérico se obtiene aparte
    con valor_numerico() cuando se necesita para calcular."""
    ...
    st.text_input(f"{label} ($)", key=campo_key, on_change=_reformatear, placeholder="0", help=ayuda or None)
...
# ----------------------------------------------------------------------
    ...
    campo_moneda(
        "Valor de la UVT", "uvt",
        ayuda="La UVT es una medida que usa la DIAN para no tener que escribir "
        "cifras en pesos en cada ley (Art. 868 E.T.). Sube cada año con el IPC. "
        "Para 2026 vale $52.374 (Resolución DIAN 000238 de 2025). Si dejas el "
        "campo vacío, se usa ese valor.",
...
# ----------------------------------------------------------------------
    ...
    campo_moneda("Casas, aptos o lotes", "casa",
                 "Lo que te costó tu casa, apto o lote cuando lo compraste (o el "
                 "avalúo catastral, si es mayor) — Art. 267 E.T. No restes nada "
                 "todavía; las deudas van en la pantalla de Pasivos.")
    campo_moneda("Dinero en bancos", "bancos",
                 "Lo que tenías en tus cuentas de ahorro, corriente o CDT, "
                 "exactamente el 31 de diciembre (Art. 261 E.T.). No es lo que "
                 "ganaste en el año, es lo que había guardado ese día.")
    campo_moneda("Carros o motos", "vehiculos",
                 "El valor de compra de tus vehículos, sin restar el desgaste "
                 "(Art. 267 E.T.). Es el precio de cuando lo compraste, no lo "
                 "que vale hoy.")
...
# ----------------------------------------------------------------------
    ...
    campo_moneda("Deudas con bancos", "deudas_bancos",
                 "Lo que te falta por pagar de tus créditos con bancos, al 31 "
                 "de diciembre (Art. 283 E.T.). Solo lo que aún debes.")
    campo_moneda("Deudas con terceros", "deudas_terceros",
                 "Deudas con personas o empresas que no son un banco. Para que "
                 "cuenten necesitas un papel que las respalde —pagaré, "
                 "contrato, etc.— (Art. 770 E.T.). Sin eso la DIAN no las "
                 "reconoce.")
...
# ----------------------------------------------------------------------
    ...
    if tiene_empleo:
        campo_moneda("Salarios y prestaciones", "salarios",
                     "Todo lo que recibiste por tu trabajo en el año: sueldo, "
                     "primas, bonos, auxilios (Art. 103 E.T.). Ponlo completo, "
                     "sin restar nada todavía.")
    if tiene_negocio:
        campo_moneda("Honorarios (Independiente)", "honorarios",
                     "Lo que facturaste en el año trabajando de forma "
                     "independiente —freelance, contratos por servicios, "
                     "negocio propio— (Art. 103 E.T.). También va completo; "
                     "los gastos del negocio se restan en la siguiente "
                     "pantalla, Deducciones.")
...
# ----------------------------------------------------------------------
    ...
    campo_moneda("Salud y Pensión obligatoria", "salud_pension",
                 "Lo que pagaste en el año, de forma obligatoria, a salud y "
                 "pensión. La ley no lo cuenta como ingreso tuyo, así que se "
                 "resta de tu base antes de calcular el impuesto (Art. 55 y "
                 "56 E.T.).")
    st.number_input(
        "Cantidad de dependientes (0 a 4)", key="num_dependientes",
        min_value=0, max_value=4, step=1,
        help="Cuántas personas dependen de ti económicamente —hijos, pareja, "
        "papás, etc.—, hasta 4. Con este número el simulador calcula dos "
        "beneficios: el 10% de tus ingresos laborales, tope 384 UVT al año "
        "(Art. 387 E.T.), y además 72 UVT fijas por cada uno, hasta 4 "
        "(Art. 387, adicionado por la Ley 2277 de 2022). No necesitas "
        "guardar facturas para esto.",
    )
    campo_moneda("Medicina prepagada", "prepagada",
                 "Lo que pagaste en el año por medicina prepagada o pólizas "
                 "de salud, para ti o tu familia. Puedes descontar hasta 192 "
                 "UVT al año (Art. 387 E.T.); si pagaste más, el simulador "
                 "solo toma el tope.")
    campo_moneda("Intereses de vivienda", "intereses",
                 "Los intereses que pagaste en el año por tu crédito "
                 "hipotecario o leasing de vivienda (Art. 119 E.T.). El tope "
                 "es 1.200 UVT al año.")
    campo_moneda("4x1000 pagado (GMF)", "gmf",
                 "Es el 4x1000 que te cobra el banco por movimientos y "
                 "retiros. Puedes descontar la mitad de lo que pagaste en el "
                 "año (Art. 115 E.T.), con el certificado del banco. Escribe "
                 "el total pagado; el simulador calcula el 50%.")
    campo_moneda("Compras con factura electrónica", "compras_facturadas",
                 "Suma tus compras del año que tengan factura electrónica, "
                 "personales o del negocio (Art. 336, numeral 5, E.T.). "
                 "Puedes descontar el 1% de ese total, hasta 240 UVT al año.")
    if obtener("perfil_honorarios", False):
        campo_moneda("Costos y gastos del negocio", "costos_gastos",
                     "Los gastos necesarios para generar tus honorarios: "
                     "arriendo del local, insumos, servicios, empleados, "
                     "etc. (Art. 107 E.T.). Se restan directamente de tus "
                     "honorarios, SIN el tope del 40%. Nota: si usas este "
                     "campo, ese ingreso ya no puede tomar además la "
                     "exención del 25% del Art. 206 —el simulador aplica "
                     "ese 25% solo a tus salarios—.")
...
# ----------------------------------------------------------------------
    ...
    campo_moneda("Aportes Cuentas AFC", "afc",
                 "Ahorros en una cuenta AFC (Ahorro para el Fomento a la "
                 "Construcción). Si los dejas quietos varios años, no pagan "
                 "impuesto (Art. 126-4 E.T.). Junto con tus pensiones "
                 "voluntarias, el tope conjunto es 30% de tus ingresos o "
                 "3.800 UVT, lo que sea menor.")
    campo_moneda("Pensiones voluntarias", "pensiones_vol",
                 "Aportes extra que hiciste a tu fondo de pensión, además de "
                 "los obligatorios. También quedan libres de impuesto "
                 "(Art. 126-1 E.T.), con el mismo tope conjunto que las "
                 "cuentas AFC.")
    campo_moneda("Donaciones (descuento tributario)", "donaciones",
                 "Donaciones a fundaciones o entidades sin ánimo de lucro "
                 "autorizadas por la DIAN. A diferencia de los otros campos, "
                 "esto no baja tu renta gravable: te devuelve el 25% de lo "
                 "donado directamente del impuesto ya calculado (Art. 257 "
                 "E.T.), hasta el 25% de ese impuesto.")
...
# ----------------------------------------------------------------------
    ...
    campo_moneda("Retenciones en la fuente del año", "retenciones",
                 "Suma de las retenciones que te hicieron en el año —las que "
                 "salen en tus certificados de ingresos o de honorarios—. "
                 "Ese dinero ya se lo diste a la DIAN por adelantado, así "
                 "que se resta de lo que debas pagar (Art. 807 E.T.).")
    campo_moneda("Anticipo pagado el año anterior", "anticipo_anterior",
                 "El anticipo de renta que pagaste en tu declaración del año "
                 "pasado. Como ya lo pagaste, se resta del total de este año "
                 "(Art. 807 E.T.).")
    campo_moneda("Impuesto neto de renta año anterior", "impuesto_anio_anterior",
                 "Opcional: el impuesto que liquidaste el año pasado, si ya "
                 "habías declarado antes. Si lo llenas, el simulador usa el "
                 "promedio de los dos últimos años cuando eso te convenga "
                 "más (Art. 807 E.T.).")
    st.selectbox(
        "N.º de veces que declaras (con esta)",
        ["Primera vez", "Segunda vez", "Tercera vez o más"],
        key="num_declaracion",
        help="Marca si esta es tu primera, segunda o tercera declaración en "
        "adelante. El porcentaje de tu anticipo depende de eso: 25%, 50% o "
        "75% (Art. 807 E.T.).",
    )
